...
def find_all_files(path_to_folder: Path, list_files: list[Path]) -> list[Path]:
    
    for new_path in path_to_folder.iterdir():
        if not new_path in IGNORED_FOLDERS:
            if new_path.is_dir():
                list_files = find_all_files(new_path, list_files)
            else:
...
def create_report_on_extensions() -> None:
...
if __name__ == "__main__":
    ...
    if len(argv) != 2:
        ...
        if not MAIN_FOLDER.is_dir():
            ...
            list_of_all_files = find_all_files(MAIN_FOLDER, [])
            create_folders()
            create_report_on_extensions()
            rename_files()
            unpack_the_archives()
